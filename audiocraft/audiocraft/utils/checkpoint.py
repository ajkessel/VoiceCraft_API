...
def resolve_checkpoint_path(sig_or_path: tp.Union[Path, str], name: tp.Optional[str] = None,
    ...
    if isinstance(sig_or_path, str):
        sig_or_path = Path(sig_or_path)
    if str(sig_or_path).startswith('//sig/'):
        sig = str(sig_or_path)[len('//sig/'):]
        path = xps_root / sig
    else:
        path = Path(AudioCraftEnvironment.resolve_reference_path(sig_or_path))
